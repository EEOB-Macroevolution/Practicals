...
1. Load the `gieger` and `phytools` package with `library()`.
...
* $offset$ this is a number that plots the points further away from the tip. This is helpful for when we want to plot multiple traits. Had we not have used offset on the second trait then it would have plotted on top of the first
...
* $model$ One of "ARD", "SYM", or "ER". ARD: all rates differ. SYM: rates between any two states do not differ (i.e $A\rightarrow B$ equals $B\rightarrow A$. ER: all rates are equal.
...
Let's analyze the evolution of beaks and webbed feet in porgs. Specifically, we might be interested in the transition rates between states. Do we see equal rates? Do we see transitions in both directions? Does have webbed feet seem to change the dynamics of transitioning between beaked state and a beakless state?
