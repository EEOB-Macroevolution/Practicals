...
We will be using the `tree.bisse()` function to simulate our data. This function takes a plethora of various arguments so it can be helpful to look at `?tree.bisse()` for documentation of these arguments. For our purposes we will be using the following arguments:
...
```
Perhaps of most note we can see a component called `tip.state`, this tells us which of the two states each tip is in. This is the type of data that researchers will have when trying to fit these types of models on a phylogeny.
```{r}
...
These data are simulated together under a BiSSE model where the diversification rates differ between the binary states. Specifically, we'll be assuming these parameters:

```{r, echo=FALSE}

sim_parameters
```
We can see that $\lambda_1$ is higher than $\lambda_0$, meaning that lineages in state $1$ will speciate at a higher rate than those in state $0$. Knowing the conditions that we simulate under can be helpful in telling us both which models may be appropriate to use and whether our estimates were accurate. 

### Fitting the BiSSE model

Now we will find out if BiSSE is able to correctly infer whether there are changes in the diversification rates on our simulated tree, by estimating the rates using maximum likelihood. We can use the `make.bisse()` function to create a $BiSSE$ model for our data. This
...
Specifically, `find.mle()` can be used to find the maximum likelihood estimate (MLE) for our parameters. This function has two main parameters:
...
We can use the `constrain()` function to constrain certain parameters of our model. This function has two main arguments:
...
To test this, we make up a secondary character for our tree.
...
We want to test whether the changes in rates are due to the observed character or to a hidden one, so first we will build a null model where the rates only depend on a hidden character. In our case we  actually know and have access to the character that actually causes a change in rates, but for the purposes of demonstration, we will pretend that we don't have that information and only have trait data for our secondary character.
 ...
 We can create and run our model with the `hisse()` function. We will be using the following arguments for this function:
...
The first step is to build a transition rate matrix for the null model. To do this we will use `TransMatMakerHiSSE()`. We will be using two arguments for this function:
...
The markings of 1, 2, or 3 don't tell us what the actual rates are, just that elements with the same number will share a rate in our model. This is similar to the `constrain()` function we used in the bisse but instead we are constraining certain transition rates to be equal. 
...
The states are ordered 0A, 1A, 0B, 1B, so we will specify that the diversification rates are identical between 0 and 1 but can differ between A and B - i.e. that states 0A and 1A share one value, and 0B and 1B share another. We chose specify these values to model the idea that changes in diversification are solely due to the hidden state and not the trait data we provide.
